				...
				$.getJSON(
					"public/webservice/internships.php", {"skill": selectedSkill}, function(result) {
						if (result.no_result) {
							...
							var table = '<table class="table table-striped"><thead><tr><th>Entrepise</th><th>Poste</th><th>Missions</th><th>Adresse</th><th>Ville</th><th>Zip_code</th><th></th></tr></thead><tbody>';
							
							for (i = 0; i < result.length; i++) {
								var internshipInfo = result[i];
								table += '<tr><td>' + internshipInfo.company + '</td><td>' + internshipInfo.name + '</td><td>' + internshipInfo.description + '</td><td>' + internshipInfo.address + '</td><td>' + internshipInfo.city + '</td><td>' + internshipInfo.zip_code + '</td>';
							}
